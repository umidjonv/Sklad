...
namespace tposDesktop
{
    public partial class MainForm : Form
    {
        ...
        private void btnOplata_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvExpense.Rows.Count != 0 && MessageBox.Show("Произвести оплату?", "Оплата", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    double sum = 0;
                    int num = 1;
                    debtPaid paidType = new debtPaid(int.Parse(lblSum.Text, System.Globalization.NumberStyles.AllowThousands));
                    ...
                    {
                        int priceTovar = (int)(((double)dgvRow.Cells["productPrice"].Value) + ((double)dgvRow.Cells["productPrice"].Value)/100*((double)(paidType.charge)));
                    ...
                    expRow.debt = (vozvrat?0:(int)debt);
                    expRow.status =  1;
                    expRow.comment = "";
                    ...
                    if (Properties.Settings.Default.isPrinter == true)
                    {
                        ...
                        EndHtml(exRow.expenseId, exRow.expSum, exRow.terminal, exRow.expType, exRow.debt, exRow.expSum, cnRow.name, paidType.nameAg);
                        //forPrinting(drPrintCol, expTable);
        ...
        private void EndHtml(int expenseId, int expSum, int terminal, int expType, int debt, int summa, string providerName, string agentName)
        {
        ...
        private string GenerateHTML(string dataHtml, int expenseId, int expSum, int terminal, int expType, int debt, int summa, string providerName, string agentName)
        {
            string temp = "";
            if (debt == 0)
            {
