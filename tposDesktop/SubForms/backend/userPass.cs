...
namespace tposDesktop
{
    public partial class userPass : DesignedForm
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (UserValues.role == "admin")
            {
                rows = table.Select("IDUser=" + idUser + " and role='" + role+ "'");
            }
            else
            {
                rows = table.Select("IDUser=" + idUser + " and password='" + hash + "' and role='" + role + "'");
            }
            // = table.Select( and password='" + hash + "' and role='user'");
            if (rows.Length != 0)
            {
